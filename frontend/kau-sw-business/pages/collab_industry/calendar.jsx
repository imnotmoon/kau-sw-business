...
const Calendar = (props) => {
  return (
    <ContentFrame title="캘린더" state={2}>
...
export const getServerSideProps = async () => {
  ...
  const getDataByDuration = axios
    .get(
      `${process.env.API_BASE_URL}/schedule?category=collab&from=${startDate}&to=${endDate}&type=duration`
    )
    .then(({ data }) => data)
    .catch((err) => console.log(err));
  const getDataByDate = axios
    .get(
      `${process.env.API_BASE_URL}/schedule?category=collab&from=${startDate}&to=${endDate}`
    )
    .then(({ data }) => data)
    .catch((err) => console.log(err));
